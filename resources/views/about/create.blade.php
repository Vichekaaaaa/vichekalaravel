...
<body>
    ...
    @section('content')
        <div class="container about-container animate__animated animate__fadeIn">
            <h1 class="text-3xl font-bold mb-6 animate__animated animate__fadeInDown">Create About</h1>
            ...
            <div class="card about-card shadow-sm animate__animated animate__fadeInUp">
                <div class="card-body">
                    <form action="{{ route('about.store') }}" method="POST" enctype="multipart/form-data" id="aboutForm">
                        @csrf
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="skillsContainer">
                                @php
                                    // Ensure old('skills') is a valid array with the expected structure
                                    $oldSkills = old('skills', [
                                        'Front-End Development' => [],
                                        'Back-End Development' => [],
                                        'Tools & Others' => [],
                                    ]);
                                    // If old('skills') exists, reorganize it to match the expected structure
                                    $categorizedSkills = [
                                        'Front-End Development' => [],
                                        'Back-End Development' => [],
                                        'Tools & Others' => [],
                                    ...
                                    @if (is_array($skills) && count($skills) > 0)
                                        @foreach ($skills as $index => $skill)
                                            <div class="skill-item animate__animated animate__fadeIn" style="animation-delay: {{ 0.1 * $loop->parent->index . '.' . $loop->index }}s;">
                                                ...
                                                <input type="text" name="skills[{{ $category }}][{{ $index }}][name]" class="form-control d-inline-block w-auto" value="{{ trim($skill['name']) }}" style="max-width: 200px;">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="experienceContainer">
                                <!-- Initial empty experience can be added dynamically -->
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="educationContainer">
                                <!-- Initial empty education can be added dynamically -->
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="hobbies" id="hobbies" class="form-control" rows="5">{{ old('hobbies') }}</textarea>
                        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                window.removeSkill = function(btn) {
                    ...
                    skillIndices[category] = skillsInCategory.length / 2; // Divide by 2 because each skill has two inputs (category and name)
                };
                ...
                window.addDescription = function(btn) {
                    ...
                    inputGroup.innerHTML = `
                        <input type="text" name="${btn.previousElementSibling.name.replace('[0]', `[${index}]`)}" class="form-control" placeholder="Description">
                        <button type="button" class="btn btn-danger btn-sm remove-desc" onclick="removeDescription(this)">Remove</button>
                    `;
