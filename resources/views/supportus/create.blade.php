...
@section('title', 'Create Support Us Section')

@section('content')
<div class="container">
    <h2 class="mb-4 animate__animated animate__fadeInDown">Create Support Us Section</h2>
    ...
    <div style="min-height: calc(100vh - 200px); padding-bottom: 100px;">
        <form action="{{ route('support-us.store') }}" method="POST" id="support-us-form">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="parent_section" id="parent_section" class="form-control">
                    <option value="" {{ old('parent_section') === '' ? 'selected' : '' }}>None (Top-level Section)</option>
                    @foreach ($sections as $parent)
                        <option value="{{ $parent }}" {{ old('parent_section') === $parent ? 'selected' : '' }}>{{ $parent }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="style" id="style" class="form-control" required>
                    <option value="" {{ old('style') === '' ? 'selected' : '' }} disabled>Select a style</option>
                    <option value="style1" {{ old('style') === 'style1' ? 'selected' : '' }}>Style 1 (Grid)</option>
                    <option value="style2" {{ old('style') === 'style2' ? 'selected' : '' }}>Style 2 (List)</option>
                </select>
            </div>
            <div class="mb-4">
                ...
                <div id="buttons-container">
                    <!-- Start with an empty container; users can add buttons if needed -->
                </div>
                <div class="mt-2" id="button-actions">
                    <button type="button" id="add-button" class="btn btn-primary animate__animated animate__bounceIn" style="animation-delay: 0.3s;">Add a Button</button>
            ...
            <button type="submit" class="btn btn-primary animate__animated animate__bounceIn" style="animation-delay: 0.5s;">Create</button>
        </form>
    </div>
</div>

<script>
    // Function to toggle visibility of the "Add a Button" button, style2 message, and icon field
    function toggleAddButtonVisibility() {
        ...
        if (styleSelect.value === 'style1') {
            ...
            iconInput.classList.remove('bg-gray-200'); // Remove disabled styling
            ...
            iconInput.classList.add('bg-gray-200'); // Add disabled styling
